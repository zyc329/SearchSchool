...
public class ScoreServiceImpl implements ScoreService {
    ...
    @Override
    public boolean add(ScoreDo scoreDo) {
        Example example = createExample(scoreDo);
        List<ScoreDo> scoreDoSelect = scoreMapper.selectByExample(example);
        if (scoreDoSelect.size() > 0) {
            return false;
        }
    ...
    @Override
    public boolean updateById(ScoreDo scoreDo) {
        HashMap<String, String> map = new HashMap<>();
        map.put("year", scoreDo.getYear());
        map.put("professionalId", scoreDo.getProfessionalId());
        ScoreDo year = BeanUtil.toBean(map,ScoreDo.class);
        Example example = createExample(year);
        List<ScoreDo> scoreDoSelect = scoreMapper.selectByExample(example);
        if (scoreDoSelect.size()>0){
            ScoreDo scoreDoItem = scoreDoSelect.get(0);
            if (scoreDoItem.getYear().equals(scoreDo.getYear())) {
                if (scoreDoItem.getScoreId().equals(scoreDo.getScoreId())){
